...
if ($u_username !== null && $u_password !== null && $u_languagecode !== null &&
    ...
    $u_fr0101 !== null && $u_fr0102 !== null && $u_fr0103 !== null && $u_fr0104 !== null && $u_fr0105 !== null
    ){
  ...
  */
  ...
  //-- Doc: perform POSTs
  $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0001', $u_languagecode, 'anonymous', $u_fr0001, $opt_ignore_blanks, $thisPageBelongsToSID);
  if ($rv === 401) {
    ...
    $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0002', $u_languagecode, 'anonymous', $u_fr0002, $opt_ignore_blanks, $thisPageBelongsToSID);
    if ($rv !== '') {
      preg_match('|.*/([^/]*)$|', $rv, $matches);
      $crid_fr0002 = $matches[1];
    }
    
    $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0101', $u_languagecode, 'anonymous', $u_fr0101, $opt_ignore_blanks, $thisPageBelongsToSID);
    if ($rv !== '') {
      preg_match('|.*/([^/]*)$|', $rv, $matches);
      $crid_fr0101 = $matches[1];
    }
    $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0102', $u_languagecode, 'anonymous', $u_fr0102, $opt_ignore_blanks, $thisPageBelongsToSID);
    if ($rv !== '') {
      preg_match('|.*/([^/]*)$|', $rv, $matches);
      $crid_fr0102 = $matches[1];
    }
    $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0103', $u_languagecode, 'anonymous', $u_fr0103, $opt_ignore_blanks, $thisPageBelongsToSID);
    if ($rv !== '') {
      preg_match('|.*/([^/]*)$|', $rv, $matches);
      $crid_fr0103 = $matches[1];
    }
    $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0104', $u_languagecode, 'anonymous', $u_fr0104, $opt_ignore_blanks, $thisPageBelongsToSID);
    if ($rv !== '') {
      preg_match('|.*/([^/]*)$|', $rv, $matches);
      $crid_fr0104 = $matches[1];
    }
    $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0105', $u_languagecode, 'anonymous', $u_fr0105, $opt_ignore_blanks, $thisPageBelongsToSID);
    if ($rv !== '') {
      preg_match('|.*/([^/]*)$|', $rv, $matches);
    ...
    if (0) {
      $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0106', $u_languagecode, 'anonymous', $u_fr0106, $opt_ignore_blanks, $thisPageBelongsToSID);
      if ($rv !== '') {
	preg_match('|.*/([^/]*)$|', $rv, $matches);
	$crid_fr0106 = $matches[1];
      }
      $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0107', $u_languagecode, 'anonymous', $u_fr0107, $opt_ignore_blanks, $thisPageBelongsToSID);
      if ($rv !== '') {
	preg_match('|.*/([^/]*)$|', $rv, $matches);
	$crid_fr0107 = $matches[1];
      }
      $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0108', $u_languagecode, 'anonymous', $u_fr0108, $opt_ignore_blanks, $thisPageBelongsToSID);
      if ($rv !== '') {
	preg_match('|.*/([^/]*)$|', $rv, $matches);
	$crid_fr0108 = $matches[1];
      }

      $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0201', $u_languagecode, 'anonymous', $u_fr0201, $opt_ignore_blanks, $thisPageBelongsToSID);
      if ($rv !== '') {
	preg_match('|.*/([^/]*)$|', $rv, $matches);
	$crid_fr0201 = $matches[1];
      }
      $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0202', $u_languagecode, 'anonymous', $u_fr0202, $opt_ignore_blanks, $thisPageBelongsToSID);
      if ($rv !== '') {
	preg_match('|.*/([^/]*)$|', $rv, $matches);
	$crid_fr0202 = $matches[1];
      }
    
      $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0301', $u_languagecode, 'anonymous', $u_fr0301, $opt_ignore_blanks, $thisPageBelongsToSID);
      if ($rv !== '') {
	preg_match('|.*/([^/]*)$|', $rv, $matches);
	$crid_fr0301 = $matches[1];
      }
      $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0302', $u_languagecode, 'anonymous', $u_fr0302, $opt_ignore_blanks, $thisPageBelongsToSID);
      if ($rv !== '') {
	preg_match('|.*/([^/]*)$|', $rv, $matches);
	$crid_fr0302 = $matches[1];
      }
      $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0303', $u_languagecode, 'anonymous', $u_fr0303, $opt_ignore_blanks, $thisPageBelongsToSID);
      if ($rv !== '') {
	preg_match('|.*/([^/]*)$|', $rv, $matches);
	$crid_fr0303 = $matches[1];
      }

      $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0401', $u_languagecode, 'anonymous', $u_fr0401, $opt_ignore_blanks, $thisPageBelongsToSID);
      if ($rv !== '') {
	preg_match('|.*/([^/]*)$|', $rv, $matches);
	$crid_fr0401 = $matches[1];
      }
      $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0402', $u_languagecode, 'anonymous', $u_fr0402, $opt_ignore_blanks, $thisPageBelongsToSID);
      if ($rv !== '') {
	preg_match('|.*/([^/]*)$|', $rv, $matches);
	$crid_fr0402 = $matches[1];
      }

      $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0501', $u_languagecode, 'anonymous', $u_fr0501, $opt_ignore_blanks, $thisPageBelongsToSID);
      if ($rv !== '') {
	preg_match('|.*/([^/]*)$|', $rv, $matches);
	$crid_fr0501 = $matches[1];
      }
      $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0502', $u_languagecode, 'anonymous', $u_fr0502, $opt_ignore_blanks, $thisPageBelongsToSID);
      if ($rv !== '') {
	preg_match('|.*/([^/]*)$|', $rv, $matches);
	$crid_fr0502 = $matches[1];
      }

      $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0601', $u_languagecode, 'anonymous', $u_fr0601, $opt_ignore_blanks, $thisPageBelongsToSID);
      if ($rv !== '') {
	preg_match('|.*/([^/]*)$|', $rv, $matches);
	$crid_fr0601 = $matches[1];
      }
      $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0602', $u_languagecode, 'anonymous', $u_fr0602, $opt_ignore_blanks, $thisPageBelongsToSID);
      if ($rv !== '') {
	preg_match('|.*/([^/]*)$|', $rv, $matches);
	$crid_fr0602 = $matches[1];
      }

      $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0801', $u_languagecode, 'anonymous', $u_fr0801, $opt_ignore_blanks, $thisPageBelongsToSID);
      if ($rv !== '') {
	preg_match('|.*/([^/]*)$|', $rv, $matches);
	$crid_fr0801 = $matches[1];
      }
      $rv = post_a_new($u_username, $u_password, $apikey, 'xkcd-hard_reboot-fr0802', $u_languagecode, 'anonymous', $u_fr0802, $opt_ignore_blanks, $thisPageBelongsToSID);
      if ($rv !== '') {
	preg_match('|.*/([^/]*)$|', $rv, $matches);
    ...
    //-- Doc: script uri
    $link = $_SERVER['SCRIPT_URI'] . "?q=$uploadersid,$u_languagecode,$crid_fr0001,$crid_fr0002,$crid_fr0101,$crid_fr0102,$crid_fr0103,$crid_fr0104,$crid_fr0105";
...
//-- Doc: handle guest params inbound
if ($guest_params !== '') {
...
if ($g_uploadersid !== null && $g_lang !== null &&
    ...
    $g_newmarkfr0101 !== null && $g_newmarkfr0102 !== null && $g_newmarkfr0103 !== null && $g_newmarkfr0104 !== null && $g_newmarkfr0105 !== null
    ){
